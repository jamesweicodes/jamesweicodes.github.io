...
export async function POST(
  ...
  try {
    ...
    if (profile.role !== "host") {
      return jsonError("Only hosts can decline bookings", 403);
    }
    ...
    if (booking.status !== "pending") {
      return jsonError("Only pending bookings can be declined", 409);
    }
    ...
    return jsonError(error instanceof Error ? error.message : "Unable to decline booking", 500);
  }
}
